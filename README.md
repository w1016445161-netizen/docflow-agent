...
> 基于 FastAPI + Vue 3 + OpenAI-compatible LLM API 构建的多格式文档解析与 RAG 问答系统。  
> 项目代号：DocFlow Agent

智文检索问答系统面向学习资料、课程论文、实验报告、项目文档等常见文档处理场景，提供从文档上传、内容解析、OCR 回退、文本分块、检索增强到模型生成的完整应用链路。

系统支持 TXT、PDF、DOCX、Markdown、Excel 等多种格式文档解析，能够对普通文档生成结构化摘要，对扫描型 PDF 自动调用 Tesseract OCR 进行识别，并基于轻量级 RAG 流程实现文档问答、参考片段展示、学习笔记生成、行动项提取和 Markdown 报告导出。

当前版本：**v0.6.0**

本项目定位为个人学习与作品集展示项目，重点实践 AI 应用开发中的文档解析、OCR 回退、大模型调用、流式输出、基础 RAG、数据库状态管理、统一异常处理、结构化日志、后端测试、Docker Compose 本地部署和前端工作台展示等工程能力。当前 RAG 使用轻量级本地实现，后续可替换为 Chroma、Milvus、FAISS、pgvector 等向量检索方案。
---
